...
def main():
    if configuration != None:
        ...
        #Inicio de Extractor
        log.warning("###############################################",True);
        log.warning("#########-INICIANDO PYTHON STREAM QUERY-########",True);
        log.warning("###############################################",True);
        ...
        if listener.memoriStatus:
            ...
            try:  
                if(configuration['stream']):
                    ...
                    for tweet in twarc.filter(track=[querys]):
...
#callProgram(log,configuration['pathfile']+'StreamFile')
main()
